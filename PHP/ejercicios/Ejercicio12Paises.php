...
// Primero hacemos un mapa rápido de capitales usando el nombre del país como clave, para no tener que recorrer todo el array cada vez.
$mapaCapitales = [];
foreach ($capitales as $nombreCapital) {
    $mapaCapitales[$nombreCapital->pais] = $nombreCapital->capital;
}
...
foreach ($paises as $pais) {
    ...
    $paisesCompletos[] = (object)[
       'nombre' => $pais->nombre, 'habitantes' => $pais->habitantes,
       'superficie' => $pais->superficie,
       'capital' => $mapaCapitales[$pais->nombre] ?? "" // si no hay capital, vacío
       ];

}

foreach ($paisesCompletos as $pais) {
    $tabla .= "<tr>";
    $tabla .= "<td>" . $pais -> nombre ."</td>";
    $tabla .= "<td style='text-align:center;'>" .$pais->capital. "</td>";
    $tabla .= "<td> " . $pais ->habitantes ."</td>";
    $tabla .= "<td> ". $pais ->superficie. " </td>";
    $tabla .= "</tr>";
}
$tabla .= "</table>";
